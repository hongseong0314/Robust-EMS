...
from utills import sample_action, initialize_state, cal_price, next_state, avg_cost_plot, load_and_generte
from model import Qnet, DQN, DDQN, DualingQnet, DualingDDQN, PER
from env import Environment_minc

def trainer(f_load, f_generation, start_day, end_day, iteration, feature, 
            start_size, learning_rate, batch_size, pos_name,
            summer_TOU, winter_TOU, battery_max,
            T, Tf, pD, gamma, freq, sample_action, buffer, coder,
            verbose=True):
    """
    f_load : 소비량 데이터프레임
    ...
    if coder == 'DQN':
        ...
        make_env_fn = lambda feature,load_data, generation_data,days, T,Tf, pD, summer_TOU, winter_TOU: Environment_minc(
                                                                                                                        feature,
                                                                                                                        load_data, 
                                                                                                                        generation_data,
                                                                                                                        days, 
        ...
        # 학습 시작
        cost_history, action_history, battery_history = agent.train(
                                                                    feature=feature, 
